...
public class RecommendBookAdapter extends RecyclerView.Adapter<RecommendBookAdapter.ViewHolder> {
    ...
    public RecommendBookAdapter(Context context, HomeBookModel homeBookModel, HomeBookContract.Presenter mHomeBookContract) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
        holder.textView.setText(mHomeBookModel.getListRecommendBook().get(position).getName());
        Glide.with(mContext)
                .load(mHomeBookModel.getListRecommendBook().get(position).getThumbnail())
                .into(holder.imageView);
        holder.llParent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(mHomeBookContract != null){
                    mHomeBookContract.onItemSelected(mHomeBookModel.getListRecommendBook().get(position).getId());
                }
            }
        });
    }
    ...
    public int getItemCount() {
        return mHomeBookModel.getListRecommendBook().size();
    }
